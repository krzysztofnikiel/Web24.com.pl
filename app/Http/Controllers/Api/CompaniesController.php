...
class CompaniesController extends Controller
{
    ...
    public function delete($id): JsonResponse
    {
        ...
        \DB::beginTransaction();
        try {
            Employee::query()->where('company_id', '=', $company->id)->update(['deleted_at' => new Carbon()]);

            $company->deleted_at = new Carbon();
            $company->save();
            \DB::commit();
        } catch (\Exception $e) {
            \DB::rollBack();
    ...
    public function put($id, Request $request): JsonResponse
    {
        ...
        if ($company === null) {
            $company = new Company();
            $company->created_at = new Carbon();
            $company->id = $id;
        } else {
            $company->updated_at = new Carbon();
        }
        try {
            $company->put($request->all());
            $company->save();
        } catch (\Exception $e) {
            Log::error($e);
            return response()->json(["success" => false], 500);
